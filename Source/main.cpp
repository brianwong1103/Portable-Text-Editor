...
BOOL DoFileOpenSave(HWND hwnd, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "布萊恩文字檔案\Brian Text Format (*.bwt)\0*.bwt\0普通檔案格式\Normal Text File (*.txt)\0*.txt\0隱藏RTF/Secret RTF (*.rtf)\0*.rtf\0\0";
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case CM_ABOUT:
					...
					break;
				case CM_LINK:
					system("start https://brianwong1103.blogspot.com");
					break;
				case CM_DLLWS:
					system("start https://github.download");
					break;
			}
			break;
		case WM_CLOSE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpCmdLine, int nCmdShow) {
	...
	hwnd = CreateWindowEx(WS_EX_CLIENTEDGE,"WindowClass","便攜式文件編輯工具/Protable Text Edit Tool Ver.1.0",WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		320,240,
